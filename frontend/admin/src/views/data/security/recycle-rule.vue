<template>
  <div>
    <n-alert type="warning" :bordered="false" class="alert-banner">
      数据回收规则用于自动回收被删除的数据，配置后系统会自动记录从对应控制器/数据表删除的数据。
    </n-alert>
    ...
    <n-modal v-model:show="showForm" preset="card" :title="editId ? '编辑规则' : '添加规则'" style="max-width: 600px">
      <n-form ref="formRef" :model="form" :rules="rules" label-placement="left" label-width="100">
        ...
        <n-form-item label="控制器" path="controller">
          <n-input v-model:value="form.controller" placeholder="如：app/admin/controller/Article" />
        </n-form-item>
        ...
        <n-form-item label="数据表" path="table_name">
          <n-input v-model:value="form.table_name" placeholder="如：swift_cms_article" />
        </n-form-item>
...
import { NButton, NSpace, NTag, NSwitch, NPopconfirm, NForm, NFormItem, NInput, NInputNumber, NIcon, useMessage } from 'naive-ui'
import { AddOutline, CreateOutline, RefreshOutline, SearchOutline } from '@vicons/ionicons5'
import { getRecycleRuleListApi, saveRecycleRuleApi, updateRecycleRuleApi, deleteRecycleRuleApi, toggleRecycleRuleStatusApi } from '@/api/recycle-rule'
...
const columns = [
  ...
  { title: '主键', key: 'primary_key', width: 80 },
  {
    ...
    render: (row: any) => row.status === 1
      ? h(NTag, { type: 'success', size: 'small' }, () => '删除监控中')
...
async function fetchData(page = pagination.page) {
  ...
  try {
    const res = await getRecycleRuleListApi({ page, limit: pagination.pageSize, keyword: keyword.value })
...
function resetForm() { form.name = ''; form.controller = ''; form.connect_id = 0; form.table_name = ''; form.primary_key = 'id'; form.status = 1 }
...
async function handleSave() {
  ...
  saving.value = true
  try {
    if (editId.value) { await updateRecycleRuleApi({ id: editId.value, ...form }); message.success('更新成功') }
    else { await saveRecycleRuleApi({ ...form }); message.success('添加成功') }
    showForm.value = false; fetchData()
  } finally { saving.value = false }
}
async function handleToggle(id: number, currentStatus: number) { await toggleRecycleRuleStatusApi(id); message.success(currentStatus === 1 ? '已禁用' : '已启用'); fetchData() }
async function handleDelete(id: number) { await deleteRecycleRuleApi(id); message.success('删除成功'); fetchData() }
async function handleBatchDelete() { for (const id of checkedIds.value) { await deleteRecycleRuleApi(id) }; message.success('批量删除成功'); fetchData() }
